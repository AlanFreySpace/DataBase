...
/**
 * 从缓冲池中删除一个页面
 * @param page_id 要删除页面的id
 * @return 如果页面存在但不能被删除返回false, 如果页面不存在或成功删除返回true
 */
bool BufferPoolManagerInstance::DeletePgImp(page_id_t page_id) {
  // 0.   确保调用了DeallocatePage!
  // 1.   在页表中查找请求删除的页P.
  // 1.   如果P不存在,返回true.
  // 2.   如果P存在, 但是pin-count不为0, 说明有人在使用该页,返回false.
  // 3.   否则, P可以被删除. 将P从页表中移除, 重置它的数据将其返还给free list.
  std::scoped_lock lock{latch_};
...
/**
 * 在缓冲池中Unpin一个页面
 * @param page_id 要unpin的页面的id
 * @param is_dirty 如果页面应标记为dirty则is_dirty为true, 否则为false
 * @return 如果页面的pin_count<= 0返回false, 否则返回true
 */
bool BufferPoolManagerInstance::UnpinPgImp(page_id_t page_id, bool is_dirty) {
  std::scoped_lock lock{latch_};
  auto iter = page_table_.find(page_id);
  // 如果不存在，即Page在磁盘中，直接返回false
  if (iter == page_table_.end()) {
    return false;
  }
  // 如果存在，即Page在缓冲池中
  ...
  page->pin_count_--;
  if (is_dirty) {  // 判断而非直接赋值是为了避免覆盖以前的true状态
    page->is_dirty_ = true;
  }
  if (page->GetPinCount() <= 0) {
